...
4. SIFT descriptors stored as `.npy` files with paths in database
...
- File paths to SIFT descriptors must be relative to prevent path traversal
...
- [ ] Hamming distance and SIFT ratio thresholds are configurable via CLI flags
