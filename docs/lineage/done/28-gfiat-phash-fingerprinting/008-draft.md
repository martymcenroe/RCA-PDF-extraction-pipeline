...
## Dependencies
- **Depends on #103** — Image extraction module must be complete (provides input images and manifest.json). The extraction issue defines the baseline `manifest.json` schema; this issue extends it with `phash` and `phash_timestamp` fields.
...
6. **Corrupted image:** Truncated/malformed image file → Graceful skip with `"phash": null`, warning logged to stderr
7. **Empty/zero-byte file:** Malformed empty file → Graceful skip with `"phash": null`, warning logged to stderr
