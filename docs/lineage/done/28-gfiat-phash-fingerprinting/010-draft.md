...
2. **Explicitly skip or ignore image entries where `phash` is null/missing** — Do not attempt Hamming distance calculation on null values
...
- `src/gfiat/fingerprint/comparison.py` — Hamming distance and pair matching (with null-check guards)
...
## Dependencies
- **Depends on #103** — Image extraction module must be complete (provides input images and manifest.json). The extraction issue defines the baseline `manifest.json` schema; this issue extends it with `phash` and `phash_timestamp` fields. **Status: Verify #103 is Done before starting development.**
- `imagehash` library (add to requirements.txt) — License: BSD-2-Clause (verified compatible with project policy)
...
6. **Very dark/black images:** Uniform dark images → Verify DCT median calculation handles edge case correctly
7. **Corrupted image:** Truncated/malformed image file → Graceful skip with `"phash": null`, warning logged to stderr
8. **Empty/zero-byte file:** Malformed empty file → Graceful skip with `"phash": null`, warning logged to stderr
9. **Comparison with null phash:** Manifest contains entries with `"phash": null` → Comparison skips these entries without TypeError
