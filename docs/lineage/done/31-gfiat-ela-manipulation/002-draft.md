...
3. Options: `--quality` (compression quality), `--scale` (visibility multiplier), `--threshold` (detection sensitivity)
...
- **Temporary Files:** Use `tempfile` module for re-compressed intermediates, cleanup after analysis
- **Region Detection:** Connected component analysis on thresholded ELA map to identify anomalous regions
- **Scoring Algorithm:** Statistical analysis of error distribution (standard deviation, histogram entropy) to compute likelihood score
...
- [ ] **Cost:** Moderate disk I/O for temporary files; cleaned up after processing
- [ ] **Legal/PII:** Images may contain sensitive content; no data leaves local system
- [ ] **Safety:** Temporary files created and deleted; no risk of data loss to originals

## Security Considerations
- Original images are never modified; only read operations performed
- Temporary re-compressed files are created in system temp directory and deleted after analysis
...
- [ ] Detect and flag uniform regions indicating potential copy-paste
- [ ] Detect and flag high-error boundaries indicating splice edges
...
- [ ] Non-JPEG files are skipped with appropriate warning
- [ ] JSON report generated with all specified fields
- [ ] Quality and scale parameters are configurable via CLI flags
...
4. Run against `tests/fixtures/ela/clean_sample.jpg`
5. Verify likelihood score is low (<0.3)

### Edge Cases to Test
- PNG file in directory (should skip with warning)
- Corrupted JPEG file (should handle gracefully)
