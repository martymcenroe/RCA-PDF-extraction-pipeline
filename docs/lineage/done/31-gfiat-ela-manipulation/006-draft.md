...
4. Compute manipulation likelihood score (0.0-1.0) using the following formula:
   - **Score = (0.4 × normalized_std_dev) + (0.3 × normalized_max_error) + (0.3 × boundary_anomaly_ratio)**
   - Where:
     - `normalized_std_dev` = std_dev(error_map) / 255
     - `normalized_max_error` = max(error_map) / 255
     - `boundary_anomaly_ratio` = count of edge pixels exceeding 2× mean error / total edge pixels
...
- **Scoring Algorithm:** Statistical analysis of error distribution using the defined formula (normalized standard deviation, normalized max error, boundary anomaly ratio) — purely heuristic, no calibration dataset required
...
- `tests/fixtures/ela/fixture_metadata.json` — Ground truth bounding boxes for automated verification
...
4. `tests/fixtures/ela/fixture_metadata.json` — Ground truth bounding boxes and baseline scores for automated verification
