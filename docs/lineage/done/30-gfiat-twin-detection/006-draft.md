...
5. System flags pairs exceeding 30% match ratio
6. System generates `twins_report.json` with flagged pairs
7. Result: User reviews report showing 3 potential clone pairs with confidence scores
...
### Scenario 6: Mirror/Flip Detection
1. User runs twins detection on directory containing horizontally flipped duplicate
2. System detects the mirrored image through SIFT matching (note: SIFT is NOT flip-invariant)
3. If flip detected, match ratio may be lower than rotation
4. Result: Report flags pair with note that geometric verification could improve confidence
...
- **FLANN Indexing:** Build KD-tree index from all descriptors for O(log n) queries per image, avoiding O(n²) pairwise loops
...
- **pHash Pre-filter:** Optional perceptual hash (ImageHash library, BSD license) to skip obviously different pairs (Hamming distance > 20)
...
7. **Mirror/Flip pair:** Image A + Image A flipped horizontally (to document detection limits)
