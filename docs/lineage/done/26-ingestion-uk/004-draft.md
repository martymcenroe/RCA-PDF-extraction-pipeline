...
2. **Data Residency: Local-Only/No External Transmission** — All downloaded data remains on the local development machine or designated internal storage. No data is transmitted to external services, cloud endpoints, or third-party systems.
...
- `src/ingestion/modules/uk.py` — New UK NDR module implementation
- `src/ingestion/modules/__init__.py` — Register UKModule in module registry
- `tests/ingestion/test_uk.py` — Unit tests with mocked responses
- `tests/ingestion/test_uk_integration.py` — Integration tests with mock server
- `tests/fixtures/uk/quadrant_search_response.json` — Static fixture for quadrant search
- `tests/fixtures/uk/well_documents_response.json` — Static fixture for document enumeration
- `tests/fixtures/uk/sample_rca.pdf` — Sample RCA PDF for download testing
- `docs/ingestion/uk-ndr.md` — Documentation of UK data source and API

## Dependencies
- Core ingestion framework must be implemented (SourceModule base class, DownloadJob) — See #TBD (verify "Done" status before starting)
- Compression utilities (zstd) available
- httpx async client configured

## Out of Scope (Future)
- **Registration/auth flow** — Deferred until we confirm if required for any data
- **Celtic Sea and Atlantic Margin quadrants** — Focus on North Sea for MVP
- **LAS file ingestion** — PDF RCA reports only for now
- **Real-time well status monitoring** — One-time ingestion, not continuous sync
...
def test_well_id_sanitization(self):  # "9/13a-A1" → "9_13a_A1"
...
- `quadrant_search_response.json` — Mock NDR search API response
- `well_documents_response.json` — Mock well document list
- `sample_rca.pdf` — Binary fixture for download verification
