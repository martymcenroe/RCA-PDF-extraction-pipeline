...
- Validation reports may contain field values; apply same data retention policies as extraction output
...
- `config/validation_rules.yaml` — Document-type-specific validation configuration
...
- [ ] Currency amounts validated as positive numbers within configured range
- [ ] VIN validated for 17-character length and valid checksum
- [ ] Mileage validated as numeric within configured range
- [ ] Cross-field date ordering enforced (loss date within policy period)
- [ ] Line item sum validation with configurable tolerance
