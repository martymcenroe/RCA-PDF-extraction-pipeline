...
- `src/gfiat/classifiers/resource_guard.py` — Decompression bomb and resource limit protection
- `tests/test_classify.py` — Unit tests for classifier
- `tests/fixtures/classify/` — Test images (rock, logo, chart samples)
- `tests/fixtures/classify/ground_truth/` — Labeled ground truth dataset for accuracy verification
- `tests/fixtures/classify/ground_truth/manifest.json` — Ground truth classification labels

## Dependencies
- **Image extraction module** (Issue #TBD) must be complete — provides manifest format
  - Expected manifest schema: `{"images": [{"path": "string", "page": "number", "type": "string", ...}]}`
  - Classifier adds `classification` and `classification_confidence` fields to each image entry
...
- **Training data collection** — Ground truth dataset is for testing only, not ML training
...
- **Confusion matrix output** — Useful for debugging but not MVP requirement
...
- [ ] Obvious non-photos (logos, charts) filtered with >90% accuracy on ground truth test dataset
