...
- `src/extraction/forgery/signature_detector.py` — Signature detection and perceptual hash comparison
...
- `src/extraction/forgery/input_sanitizer.py` — Image/PDF input validation (client for sandbox service)
- `src/extraction/forgery/sandbox_client.py` — Client for communicating with forgery-sandbox-service
...
- `tests/extraction/forgery/fixtures/malicious/` — Corrupted/malicious document test fixtures (zip bombs, polyglot files, malformed headers)
- `config/forgery_thresholds.yaml` — Configurable detection thresholds
- `services/forgery-sandbox-service/` — Standalone microservice for sandboxed image processing
- `services/forgery-sandbox-service/Dockerfile` — Ephemeral container for sandboxed processing
- `services/forgery-sandbox-service/seccomp-profile.json` — Seccomp profile restricting syscalls
...
- **Feature point matching (SIFT/ORB)** — Requires raw signature storage; using perceptual hashing for legal compliance
...
- [ ] Signature comparison using perceptual hash similarity achieves ≥85% accuracy on reference match test set
