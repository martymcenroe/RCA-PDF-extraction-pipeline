# Issue #19: Forgery Detection: Seals and Signatures
...
## Labels
`machine-learning`, `security`, `feature`, `compliance`

## Effort Estimate
**XL** â€” Requires model tuning, image processing pipeline, and extensive validation testing.
...
2. System compares signature perceptual hash characteristics against stored reference hash
3. Significant deviation detected in hash similarity score
...
5. Compare against reference signature perceptual hashes when available (using pHash/dHash via `imagehash` library)
...
2. Reference signatures MUST be stored as one-way perceptual hashes (pHash/dHash), NOT raw images, to reduce liability
...
- **Signature Detection:** CNN-based signature detection model; stroke analysis using image processing; perceptual hash comparison (pHash/dHash via `imagehash`) for signature matching against stored reference hashes
...
- **Input Sandboxing:** Sandboxed processing implemented as a **standalone microservice** (`forgery-sandbox-service`) to isolate high-privilege container operations from the main application; main application communicates via internal message queue (no Docker socket access required in main app)
...
- All image processing occurs in sandboxed ephemeral containers via isolated microservice

## Files to Create/Modify
- `src/extraction/forgery/__init__.py` â€” Module initialization
- `src/extraction/forgery/seal_detector.py` â€” Seal detection and analysis
- `src/extraction/forgery/signature_detector.py` â€” Signature detection and perceptual hash comparison
- `src/extraction/forgery/manipulation_detector.py` â€” Artifact and tampering detection
- `src/extraction/forgery/confidence_scorer.py` â€” Ensemble confidence scoring
- `src/extraction/forgery/input_sanitizer.py` â€” Image/PDF input validation (client for sandbox service)
- `src/extraction/forgery/sandbox_client.py` â€” Client for communicating with forgery-sandbox-service
- `src/extraction/validators/authenticity_validator.py` â€” Pipeline integration validator
- `src/extraction/models/forgery_result.py` â€” Data models for detection results
- `tests/extraction/forgery/` â€” Test suite for all forgery detection components
- `tests/extraction/forgery/fixtures/malicious/` â€” Corrupted/malicious document test fixtures (zip bombs, polyglot files, malformed headers)
- `config/forgery_thresholds.yaml` â€” Configurable detection thresholds
- `services/forgery-sandbox-service/` â€” Standalone microservice for sandboxed image processing
- `services/forgery-sandbox-service/Dockerfile` â€” Ephemeral container for sandboxed processing
- `services/forgery-sandbox-service/seccomp-profile.json` â€” Seccomp profile restricting syscalls
...
- `imagehash` for perceptual signature hashing (pHash/dHash)
- Docker for sandboxed processing containers
- **Issue #0087** â€” Existing validation pipeline must support async validators (Status: Complete)

## Out of Scope (Future)
- **Real-time video verification** â€” Live notarization verification deferred
- **Blockchain seal verification** â€” Digital notary integration for future
- **Multi-language seal OCR** â€” Focus on English seals initially
- **Signature biometric analysis** â€” Pressure/timing analysis requires specialized hardware
- **Automatic reference database building** â€” Manual curation for MVP
- **GPU acceleration** â€” CPU inference sufficient for MVP throughput
- **Feature point matching (SIFT/ORB)** â€” Requires raw signature storage; using perceptual hashing for legal compliance

## Acceptance Criteria
- [ ] Seal detector identifies seal presence with â‰¥90% accuracy on test dataset
- [ ] Seal detector flags known forgery artifacts (perfect geometry, digital fonts) with â‰¥85% recall
- [ ] Seal clarity verification uses Laplacian variance threshold > 100
- [ ] Signature detector identifies signature presence on designated lines with â‰¥95% accuracy
- [ ] Signature detector flags typed/stamped signatures as non-handwritten
- [ ] Signature comparison using perceptual hash similarity achieves â‰¥85% accuracy on reference match test set
- [ ] Manipulation detector identifies copy/paste artifacts in synthetic test images
- [ ] Manipulation detector detects â‰¥90% of Error Level Analysis discrepancies on test set 4
...
- [ ] Pipeline latency increase â‰¤500ms per document for standard analysis (on c5.xlarge or equivalent)
- [ ] False positive rate â‰¤5% on legitimate document test set
- [ ] Integration tests pass with existing validation pipeline
- [ ] All image processing occurs in sandboxed microservice with no network access
- [ ] No signature/biometric data transmitted to external APIs
- [ ] Reference signatures stored as perceptual hashes only
- [ ] Input sanitizer rejects 100% of malicious test fixtures (zip bombs, polyglot files, malformed headers)
...
- [ ] Input sanitization and sandboxed microservice implemented
...
- [ ] Create ADR for signature storage decision (perceptual hashes vs. raw images)
- [ ] Create ADR for sandbox microservice architecture decision
...
- Test microservice communication failure scenarios

### Static Fixtures for Offline Development
- **Legitimate Documents:** Standard contracts, notarized forms, signed applications
- **Synthetic Forgeries:** Perfect-circle seals, typed signatures, copy-paste artifacts
- **Malicious Document Set:** Zip bombs, polyglot files (PDF/ZIP hybrids), files with malformed headers, oversized images, corrupted metadata
...
- Submit zip bombs and polyglot files to verify rejection
