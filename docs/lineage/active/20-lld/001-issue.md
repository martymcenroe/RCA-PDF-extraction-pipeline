...
3. Mileage reasonableness check against vehicle age using VIN 10th digit decoding (local regex logic, no external API)
...
- **Auto Claims:** VIN (required), mileage (required), date_of_loss (required), policy_number (required)
- **Property Claims:** property_address (required), date_of_loss (required), policy_number (required)
- **General Claims:** date_of_loss (required), policy_number (required), claimant_name (required)
...
- **Pydantic Error Handling:** Custom logging implementation explicitly strips the `input` field from Pydantic `ValidationError` messages before writing to any non-secure log stream to prevent PII leakage
- **VIN Year Decoding:** Vehicle age calculated from VIN 10th digit using local regex/logic decoding (no external API required)
...
- [ ] Mileage cross-validated against vehicle age derived from VIN 10th digit (WARNING if mileage exceeds 15,000 miles per year of vehicle age)
...
- [ ] Required field presence checked based on document type configuration (e.g., auto claims require VIN, mileage)
...
- [ ] Cross-field validation rule engine implemented (including VIN year vs mileage logic)
...
- Test VIN 10th digit year decoding with various model years (1980-2024)
...
- Test mileage vs vehicle age cross-validation at boundary (exactly 15,000 miles/year)
...
## Labels
`feature`, `pipeline`, `quality-assurance`, `compliance`
...
<sub>**Gemini Review:** APPROVED | **Model:** `gemini-3-pro-preview` | **Date:** 2026-01-31 | **Reviews:** 3</sub>
