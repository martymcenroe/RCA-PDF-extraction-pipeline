# Issue #21: Alberta AER Data Ingestion Module
...
2. System normalizes UWI formats (e.g., `100/06-12-034-05W4/0` â†’ `100_06_12_034_05W4_0`)
...
| Max raw storage | 30 GB | 15,000 Ã— 2 MB |
| Post-compression | ~22.5 GB | ~75% with zstd |
| Bandwidth per run | Variable | Depends on `--limit` flag |
| Typical batch (limit=50) | 100 MB | 50 Ã— 2 MB |

### Rate Limit Impact
- At 1 req/sec: 50 documents â‰ˆ 2 minutes discovery + 50 seconds download
- Full corpus (15,000 docs): ~8.5 hours wall time

## Legal & Compliance

### Scraping Compliance Verification
- **AER Acceptable Use Policy:** Verified â€” AER provides public API access for non-confidential well data
- **robots.txt:** Checked at https://www.aer.ca/robots.txt â€” API endpoints not disallowed
...
### Data Residency
- **Requirement:** Canadian government data â€” Local-Only processing
...
- `src/ingestion/modules/alberta.py` â€” New module implementing AER data ingestion
- `src/ingestion/modules/__init__.py` â€” Register AlbertaModule
- `tests/ingestion/test_alberta.py` â€” Unit tests for UWI parsing, filtering, confidentiality checks
- `tests/ingestion/test_alberta_integration.py` â€” Integration tests with mock AER server
- `tests/fixtures/aer_sample_response.json` â€” Sample API response fixture for offline development
- `docs/ingestion/alberta.md` â€” Documentation for AER data source and module usage
...
## Out of Scope (Future)
- Historical data quality assessment â€” separate data quality initiative
- Multi-province support (BC, Saskatchewan) â€” separate issues per province
- LAS file ingestion â€” focus on RCA PDFs first
- Core photo retrieval â€” document-focused for MVP
- Proxy rotation â€” not required given conservative rate limiting (see Questions section)
...
- [ ] Rate limiting maintains â‰¤1 request/second to AER
