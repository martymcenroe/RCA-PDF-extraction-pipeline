# Issue #22: Australian WAPIMS Data Ingestion Module
...
1. User runs ingestion with batch-eligible count (â‰¥10 wells)
...
5. **Sanitize basin and well names to remove characters invalid for filesystems or that could lead to directory traversal before path creation**

## Resource Estimation
- **API Costs:** Zero (public government portal)
- **Storage Footprint:** Estimated < 500MB for initial MVP runs (typical RCA PDFs are 1-5MB each, compressed ~30% with zstd)
- **Network:** Batch downloads reduce request count; individual fallback stays within rate limits
- **Fits within existing project storage quotas**
...
- **Batch System:** Basket creation â†’ add items â†’ submit â†’ poll â†’ extract
...
- **Path Sanitization:** All basin and well name strings are sanitized before filesystem path construction to prevent directory traversal attacks

## Legal & Data Handling
- **Data Residency:** Local-Only processing; no scraped data is transmitted to external third-party services
- **Data Source:** Western Australian Government public data (DMIRS)
- **Compliance:** All data accessed is publicly released under WAPIMS data release policies

## Effort Estimate
**T-Shirt Size: L** â€” Complex batch polling logic, fallback mechanisms, and ZIP extraction handling

## Files to Create/Modify
- `src/ingestion/modules/australia.py` â€” New module implementing AustraliaModule class
- `src/ingestion/modules/__init__.py` â€” Register australia module
- `tests/ingestion/test_australia.py` â€” Unit and integration tests
- `tests/fixtures/wapims/` â€” Mock responses for WAPIMS API
- `docs/ingestion/australia.md` â€” Module documentation and API notes
...
## Out of Scope (Future)
- **LAS file ingestion** â€” Focus on PDFs for MVP, LAS parsing deferred
- **Registration/authentication** â€” Public data only for now
- **Real-time basin monitoring** â€” No webhook/notification support
- **Geothermal data** â€” WAPIMS includes geothermal; petroleum focus only for MVP
...
- [ ] Basin and well names are sanitized before path construction
- [ ] Manifest entries include basin, well_name, permit, operator metadata
- [ ] `--dry-run` lists documents without downloading
- [ ] Unit tests cover basin search, RCA filtering, batch polling, path sanitization
...
- [ ] Path sanitization for all user-derived path components
...
- **Path traversal attempt:** Test with malicious well names containing `../`
