...
Update Reason: Initial LLD creation for historical reference database feature
-->
...
- [ ] Should SIFT descriptors be stored in a single directory or organized by source PDF?
- [ ] What is the expected maximum database size for the initial deployment?
...
|------|-------------|-------------|
| `src/gfiat/db/__init__.py` | Add | Database module initialization and exports |
| `src/gfiat/db/schema.py` | Add | SQLite schema definition and migrations |
| `src/gfiat/db/ingest.py` | Add | Ingestion logic for historical reports |
| `src/gfiat/db/query.py` | Add | Query functions for similarity search |
| `src/gfiat/db/validation.py` | Add | Input path sanitization and validation |
| `src/gfiat/db/__main__.py` | Add | CLI entry point for database operations |
...
def store_sift_file(descriptors: np.ndarray, base_dir: str, image_id: int) -> str:
    """Store SIFT descriptors as .npy file. Returns relative path."""
    ...

# src/gfiat/db/query.py
def query_by_phash(
    phash: str,
    db_path: str,
    threshold: int = 5
...
3. Get/create database connection
4. Scan directory recursively for image files
5. FOR each image file:
   a. Extract metadata (source PDF, page, index) from path structure
   b. Check if source PDF already exists in database
   c. IF exists AND mode == "skip-existing":
      - Log skip, continue
   d. IF exists AND mode == "force":
      - Delete existing entries for this PDF
   e. TRY:
      - Extract pHash from image
      - Extract SIFT descriptors from image
      - Store SIFT as .npy file with relative path
      - Insert/update record in database
   f. EXCEPT (file corruption, read error):
      - Log error to ingest_errors.log
      - Continue processing remaining files
6. Return IngestStats with counts
```
...
Development: Local SQLite file in `data/gfiat.db`, SIFT files in `data/sift/`
...
# Run with coverage
poetry run pytest tests/test_db_*.py -v --cov=src/gfiat/db
```
