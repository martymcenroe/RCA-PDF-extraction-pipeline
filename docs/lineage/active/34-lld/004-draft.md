...
5. Group images by source PDF
6. FOR each source PDF:
   ...
   c. IF exists AND mode == "force":
      - Delete ALL existing entries for this PDF (ONCE, before processing images)
...
7. Return IngestStats with counts
```
...
1. User runs `python -m gfiat.db ingest ./extracted/`
...
### CLI Interface
1. `python -m gfiat.db ingest <directory> [--skip-existing|--update-existing|--force]` — add images to database
2. `python -m gfiat.db query --phash <hash> [--threshold N]` — find similar by pHash
3. `python -m gfiat.db query --image <path> [--ratio N]` — find similar by image file
4. `python -m gfiat.db stats` — show database statistics
