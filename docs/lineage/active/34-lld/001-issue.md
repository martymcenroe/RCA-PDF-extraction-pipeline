# Issue #34: Historical Reference Database for Cross-Report Image Comparison
...
### CLI Interface
1. `python -m src.gfiat.db ingest <directory> [--skip-existing|--update-existing|--force]` â€” add images to database
2. `python -m src.gfiat.db query --phash <hash> [--threshold N]` â€” find similar by pHash
3. `python -m src.gfiat.db query --image <path> [--ratio N]` â€” find similar by image file
4. `python -m src.gfiat.db stats` â€” show database statistics
...
- `src/gfiat/db/__init__.py` â€” Database module initialization
- `src/gfiat/db/schema.py` â€” SQLite schema and migrations
- `src/gfiat/db/ingest.py` â€” Ingestion logic for historical reports
- `src/gfiat/db/query.py` â€” Query functions for similarity search
- `src/gfiat/db/validation.py` â€” Input path sanitization and validation
- `src/gfiat/db/__main__.py` â€” CLI entry point
- `tests/test_db_ingest.py` â€” Ingestion tests
- `tests/test_db_query.py` â€” Query tests
- `tests/test_db_validation.py` â€” Path validation tests

## Dependencies
- Issue #23 must be completed first (Data Ingestion provides source PDFs and extraction)

## Out of Scope (Future)
- **FAISS integration** â€” deferred until scale requires it (thousands of images)
- **Automatic re-indexing** â€” manual ingest only for MVP
- **Web UI for database browsing** â€” CLI only
- **Distributed/sharded storage** â€” single SQLite file for now
- **Encryption at rest (SQLCipher)** â€” evaluate if well names require protection
- **Database migration tooling** â€” defer until schema changes required
...
## Effort Estimate
**Size: M** (Medium) â€” New module with schema, ingestion, query logic, and CLI
