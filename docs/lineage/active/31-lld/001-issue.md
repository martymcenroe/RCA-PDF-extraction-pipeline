# Issue #31: Error Level Analysis (ELA) for Image Manipulation Detection
...
### Scenario 6: Image Exceeds Timeout
1. User runs ELA analysis on directory containing a very large image
2. System begins processing the large image
3. Processing exceeds configured timeout threshold (default: 60 seconds)
4. System logs warning: "Skipping {filename}: processing timeout exceeded ({timeout}s)"
5. System continues processing remaining images
6. Result: Report includes skipped file with timeout reason
...
4. Compute manipulation likelihood score (0.0-1.0) using the following formula:
   - **Score = (0.4 Ã— normalized_std_dev) + (0.3 Ã— normalized_max_error) + (0.3 Ã— boundary_anomaly_ratio)**
   - Where:
     - `normalized_std_dev` = std_dev(error_map) / 255
     - `normalized_max_error` = max(error_map) / 255
     - `boundary_anomaly_ratio` = count of edge pixels exceeding 2Ã— mean error / total edge pixels
...
- **Scoring Algorithm:** Statistical analysis of error distribution using the defined formula (normalized standard deviation, normalized max error, boundary anomaly ratio) â€” purely heuristic, no calibration dataset required
- **Batch Processing:** Parallel processing with `concurrent.futures` for directory analysis
- **Timeout Handling:** Per-image timeout using `concurrent.futures.wait()` with timeout parameter to prevent hanging on very large images
...
- [x] **Cost:** Moderate disk I/O for temporary files; cleaned up after processing via try/finally
- [x] **Legal/PII:** Images may contain sensitive content; no data leaves local system
...
- `src/gfiat/analyzers/ela.py` â€” Core ELA analysis implementation
- `src/gfiat/analyzers/__init__.py` â€” Export ELA analyzer
- `src/gfiat/cli/analyze.py` â€” Add `ela` subcommand to CLI
- `src/gfiat/models/ela_report.py` â€” Data classes for ELA results
- `tests/test_ela.py` â€” Unit tests for ELA analysis
- `tests/fixtures/ela/` â€” Test images (manipulated and clean samples)
- `tests/fixtures/ela/fixture_metadata.json` â€” Ground truth bounding boxes for automated verification

## Dependencies
- OpenCV (`opencv-python`) â€” likely already installed for image processing
- NumPy â€” likely already installed

## Out of Scope (Future)
- **Video frame analysis** â€” ELA for video deferred to future issue
- **Machine learning classifier** â€” ML-based manipulation detection beyond heuristics
- **FFT combination** â€” Combining with frequency analysis for stronger signal (mentioned in brief, separate issue)
- **Interactive GUI** â€” Visual tool for exploring ELA results
- **Cloud processing** â€” Batch processing via cloud infrastructure
- **Calibration dataset** â€” Score is purely heuristic; calibrated ML scoring is future scope

## Acceptance Criteria
- [ ] Generate ELA map for each JPEG image in input path
- [ ] When running against `tests/fixtures/ela/manipulated_sample.jpg`, system outputs a bounding box overlapping the known manipulated region by >50% (per fixture_metadata.json)
- [ ] When running against `tests/fixtures/ela/clean_sample.jpg`, likelihood score is below baseline threshold documented in fixture_metadata.json
- [ ] Output ELA overlay images with `_ela` suffix alongside analysis
- [ ] Flag images with anomalous error distributions (likelihood exceeds threshold documented in fixture_metadata.json)
...
- [ ] Baseline scores for test fixtures are determined during implementation and documented in fixture_metadata.json
...
1. `tests/fixtures/ela/manipulated_sample.jpg` â€” Known spliced region at documented coordinates
2. `tests/fixtures/ela/clean_sample.jpg` â€” Unmodified image for baseline
3. `tests/fixtures/ela/high_detail_sample.jpg` â€” Text/edges for false positive testing
4. `tests/fixtures/ela/fixture_metadata.json` â€” Ground truth bounding boxes and baseline scores for automated verification

### Manual Verification
1. Run against `tests/fixtures/ela/manipulated_sample.jpg`
2. Verify ELA overlay highlights the spliced region
3. Verify JSON report shows elevated likelihood score
4. Verify detected bounding box overlaps known region by >50%
5. Run against `tests/fixtures/ela/clean_sample.jpg`
6. Verify likelihood score is below documented baseline
