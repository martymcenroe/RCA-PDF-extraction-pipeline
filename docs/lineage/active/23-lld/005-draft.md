...
The following directories must be created as part of implementation (they do not exist yet):
- `src/ingestion/` — New package directory (parent `src/` exists)
- `src/ingestion/modules/` — Subpackage for source modules
...
**Implementation Note:** Create `src/ingestion/` and `src/ingestion/modules/` directories before adding files.
...
```bash
# Run all unit tests
poetry run pytest tests/unit/test_ingestion_*.py -v --cov=src/ingestion --cov-report=term-missing
