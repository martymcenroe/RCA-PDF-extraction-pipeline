...
|------|-------------|-------------|
| `src/ingestion.py` | Add | All ingestion framework code: base classes, controller, storage manager, USGS module, CLI |
| `tests/unit/test_ingestion.py` | Add | Unit tests for all ingestion components |
| `tests/integration/test_ingestion_integration.py` | Add | Integration tests |
...
- `src/` ✓ exists (for `src/ingestion.py`)
- `tests/unit/` ✓ exists (for `tests/unit/test_ingestion.py`)
- `tests/integration/` ✓ exists (for `tests/integration/test_ingestion_integration.py`)
...
class IngestionStats(TypedDict):
    """Statistics for status command output."""
    source: str               # Source module name
    completed_count: int      # Number of successfully downloaded items
    failed_count: int         # Number of failed items
    pending_count: int | None # Number of pending items (if known)
    last_run: str | None      # ISO timestamp of last ingestion
    total_bytes: int          # Total compressed bytes stored
```

### 2.4 Function Signatures

```python
# === src/ingestion.py - Path Sanitization ===
...
class IngestionController:
    ...
    def get_status(self, source_name: str) -> IngestionStats:
        """Get ingestion statistics for a source from checkpoint and manifest."""
        ...

# === src/ingestion.py - USGS Module ===
...
# === src/ingestion.py - CLI Commands ===

def cli_status(source: str) -> None:
    """Display ingestion statistics for a source."""
    ...
```
...
STATUS COMMAND FLOW:
1. Parse CLI arguments (source)
2. Initialize IngestionController
3. Load checkpoint from data/state/{source}.json
4. Load manifest from data/raw/{source}/manifest.json
5. Compute statistics:
   ...
   - last_run = checkpoint.last_updated
6. Format and print statistics to stdout
...
* **Key Decisions:**
  - Single-file module for simplicity (all code in `src/ingestion.py`)
  ...
  - Path sanitization functions isolated at top of module for security-critical code visibility
  - User-Agent header set to "USGS-Ingestion-Bot/1.0" for polite identification
...
| Module Structure | Package with submodules, single file | Single file | Simpler for MVP, no directory creation needed |
...
8. **R8:** `status` command shows progress statistics (completed, failed, total bytes)
...
| Package structure | Better organization | Requires new directories | **Rejected** for MVP |
| Single file module | No new directories needed, simpler | Less organized | **Selected** |

**Rationale:** Prioritizing simplicity and debuggability for MVP. Single file keeps all code together and avoids directory creation issues. Parallel downloads can be added as optimization later with proper semaphore-based rate limiting.
...
| T160 | test_status_command_output | Displays stats to stdout | RED |
...
- [ ] Test files created at: `tests/unit/test_ingestion.py`
...
| 160 | Status command output | Auto | Checkpoint file exists | Displays stats to stdout | stdout contains correct counts |

### 10.2 Test Commands

```bash
# Run all unit tests
poetry run pytest tests/unit/test_ingestion.py -v --cov=src/ingestion --cov-report=term-missing

# Run only fast/mocked tests (exclude live)
poetry run pytest tests/unit/test_ingestion.py -v -m "not live"

# Run live integration test (USGS, limit=1)
poetry run pytest tests/integration/test_ingestion_integration.py -v -m live

# Smoke test commands
python -m src.ingestion ingest usgs --limit 1 --dry-run
python -m src.ingestion ingest usgs --limit 1
python -m src.ingestion status usgs
```
...
- Every risk mitigation in Section 11 should have a corresponding function in Section 2.4 ✓
  ...
  - `get_status()` for status command functionality
...
|----|---------|--------------|
| G1.1 | "Open Question about proxy configuration should be resolved" | YES - Marked resolved in Section 1, uses httpx native env var support |
| G1.2 | "Missing test scenario for R8 (status command)" | YES - Added T160 in Section 10.0 and scenario 160 in Section 10.1 |
| G1.3 | "Coverage is 91.6%, below 95% threshold" | YES - Added T160 to achieve 12/12 = 100% requirement coverage |
| G1.4 | "Suggestion: Define User-Agent header explicitly" | YES - Added to Section 2.6 Technical Approach |

### Review Summary

| Review | Date | Verdict | Key Issue |
|--------|------|---------|-----------|
| Gemini #1 | 2026-01-31 | REVISE | Missing test for status command (R8) |
