...
|------|-------------|-------------|
| `src/rca_agent/ingestion/__init__.py` | Add | Package init with public exports |
| `src/rca_agent/ingestion/core.py` | Add | Base classes, controller, storage manager |
| `src/rca_agent/ingestion/sanitize.py` | Add | Path sanitization utilities |
| `src/rca_agent/ingestion/modules/__init__.py` | Add | Modules package init |
| `src/rca_agent/ingestion/modules/usgs.py` | Add | USGS CRC source module implementation |
| `src/rca_agent/ingestion/cli.py` | Add | Click-based CLI interface |
| `tests/unit/__init__.py` | Modify | Add ingestion test imports |
| `tests/unit/test_ingestion_core.py` | Add | Unit tests for core components |
| `tests/unit/test_ingestion_sanitize.py` | Add | Unit tests for path sanitization |
| `tests/unit/test_ingestion_usgs.py` | Add | Unit tests for USGS module |
...
- All "Modify" files must exist in repository ✓ (`pyproject.toml`, `.gitignore`, `tests/unit/__init__.py`)
- All "Delete" files must exist in repository ✓ (none)
- All "Add" files must have existing parent directories ✓ (`src/rca_agent/` exists, `tests/unit/` exists, `tests/integration/` exists)
...
```python
# === src/rca_agent/ingestion/sanitize.py ===
...
# === src/rca_agent/ingestion/core.py ===
...
# === src/rca_agent/ingestion/modules/usgs.py ===
...
* **Module:** `src/rca_agent/ingestion/`
...
1. **R1:** `python -m src.rca_agent.ingestion ingest usgs --limit 5` downloads 5 RCA PDFs from USGS
...
- [ ] Test files created at: `tests/unit/test_ingestion_*.py`
...
```bash
# Run all unit tests
poetry run pytest tests/unit/test_ingestion_*.py -v --cov=src/rca_agent/ingestion --cov-report=term-missing

# Run only sanitization tests
poetry run pytest tests/unit/test_ingestion_sanitize.py -v

# Run only fast/mocked tests (exclude live)
poetry run pytest tests/unit/test_ingestion_*.py -v -m "not live"

# Run live integration test (USGS, limit=1)
poetry run pytest tests/integration/test_ingestion_integration.py -v -m live

# Smoke test commands
python -m src.rca_agent.ingestion ingest usgs --limit 1 --dry-run
python -m src.rca_agent.ingestion ingest usgs --limit 1
python -m src.rca_agent.ingestion status
```
