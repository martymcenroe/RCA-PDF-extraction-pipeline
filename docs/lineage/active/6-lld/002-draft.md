...
- [x] NA representation: `NA` string (R-compatible, pandas via `na_values=['NA']`)
- [x] Below-detection handling: NA with flag column (Option A)
- [ ] Should `permeability_below_detection` be set TRUE when only one of air/klink is below detection, or both?
...
|------|-------------|-------------|
| `src/extractors/core_analysis.py` | Modify | Add `to_analytics_format()` function with transformation logic |
| `src/utils/csv_sanitizer.py` | Add | New module for CSV injection prevention utilities |
| `src/cli/extract.py` | Modify | Add `--analytics` flag to CLI |
...
| `docs/output-formats.md` | Modify | Document new analytics format and schema |
...
class AnalyticsRow(TypedDict):
    ...
    permeability_below_detection: bool    # TRUE if permeability was '+' or '<0.0001'
...
# src/extractors/core_analysis.py
...
6. IF analytics flag is True:
   a. TRY to_analytics_format(rows)
      - FOR each row:
        ...
        - Transform permeability: '+' or '<0.0001' → NA, set flag
...
* **Module:** `src/extractors/core_analysis.py` (transformation), `src/utils/csv_sanitizer.py` (security)
...
| T150 | test_analytics_no_empty_cells | No empty cells in output | RED |
| T160 | test_analytics_numeric_columns_parseable | Pandas parses as float64 | RED |
| T170 | test_transformation_failure_partial_success | Core outputs preserved on failure | RED |
...
| 150 | No empty cells | Auto | Analytics CSV file | All cells non-empty | Check via pandas |
| 160 | Pandas compatibility | Auto | Analytics CSV file | Numeric cols as float64 | dtype check |
| 170 | Partial success on failure | Auto | Mocked transform error | Core outputs exist | Files present |
...
# Run with coverage
poetry run pytest tests/test_analytics_format.py tests/test_csv_sanitizer.py --cov=src/extractors --cov=src/utils --cov-report=term-missing
```
...
- `src/extractors/core_analysis.py` ✓ (Section 2.1)
- `src/utils/csv_sanitizer.py` ✓ (Section 2.1)
- `src/cli/extract.py` ✓ (Section 2.1)
