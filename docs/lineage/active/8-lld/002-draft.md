...
| `.github/workflows/ci.yml` | Modify | Add comparison step after both pipelines run |
...
| `docs/reports/0008/implementation-report.md` | Add | Implementation documentation |
...
8. Check if Spec has more rows than Extended
   - IF remaining rows THEN add to issues list
9. Report results:
...
12. CI workflow includes comparison step after both pipelines run
...
flowchart TD
    subgraph CI["CI Workflow"]
        A[Run Spec Pipeline] --> B[Run Extended Pipeline]
        B --> C[compare_outputs.py]
    end
    
    subgraph Comparison["Comparison Script"]
        C --> D{Validate Paths}
        D -->|Invalid| E[Exit 1: Path Error]
        D -->|Valid| F{Files Exist?}
        F -->|No| G[Exit 1: File Not Found]
        F -->|Yes| H[Stream & Compare]
        H --> I{Columns Match?}
        I -->|No| J[Exit 1: Missing Columns]
        I -->|Yes| K{Values Match?}
        K -->|No| L[Exit 1: Value Mismatch]
...
|--------|------|---------|-----------|
| Initial Draft | - | PENDING | Awaiting review |
