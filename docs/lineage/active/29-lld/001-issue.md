# Issue #29: G-FIAT: SIFT Feature Extraction Engine
...
2. Write `.npy` files incrementally per-image (process image â†’ write file â†’ release memory) to prevent RAM accumulation
...
2. Options:
   ...
   - `--output-dir PATH` (default: `<input_path>/sift_descriptors/`) â€” validated for path traversal
   - `--dry-run` â€” preview processing without extracting features
...
- `src/gfiat/fingerprint/sift.py` â€” Core SIFT extraction logic
- `src/gfiat/fingerprint/path_validator.py` â€” Output path sanitization and validation
- `src/gfiat/fingerprint/__init__.py` â€” Register SIFT as available method
- `src/gfiat/fingerprint/cli.py` â€” Add `--method sift` and `--dry-run` options
- `src/gfiat/manifest/schema.py` â€” Extend schema for SIFT metadata
- `tests/fingerprint/test_sift.py` â€” Unit tests for SIFT extraction
- `tests/fingerprint/test_path_validator.py` â€” Unit tests for path validation
- `tests/fixtures/` â€” Add test images with known keypoint counts

## Dependencies
- OpenCV headless (`opencv-python-headless>=4.5.0`) â€” standard BSD-licensed package, SIFT included since 4.4.0
- NumPy (already in project)
- Extraction phase must be complete (images available in `./extracted/`)

## Out of Scope (Future)
- **Keypoint matching** â€” separate issue for comparing descriptors between images
- **SURF/ORB alternatives** â€” may add as options in future iteration
- **SQLite storage** â€” evaluate after measuring `.npy` performance at scale
- **GPU acceleration** â€” CUDA SIFT for large batch processing
- **Keypoint visualization** â€” debug tool to overlay keypoints on images
...
### Edge Cases to Test
1. **Rotation invariance:** Extract features from original and 45Â° rotated version, verify similar keypoint count
