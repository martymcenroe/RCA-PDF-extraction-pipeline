...
cp ./data/output/spec/core_analysis.csv /tmp/baseline_before.csv

# === PHASE 2: RUN UNIT TESTS ===
poetry run --directory . pytest tests/test_footnote_symbols.py -v

# === PHASE 3: GENERATE NEW OUTPUT ===
# Use the WORKTREE code to generate output
poetry run --directory . python -m src.core_analysis ./data/output/spec/W20552.db --output /tmp/issue4 --original-headers
...
# diff ./data/output/spec/core_analysis.csv /tmp/issue4/core_analysis.csv
# echo $?  # MUST output 0
...
1. If all checks pass, copy new output to baseline:
   ```bash
   cp /tmp/issue4/core_analysis.csv ./data/output/spec/core_analysis.csv
   ```
